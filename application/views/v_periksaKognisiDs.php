    ...
    <section class="content">
	  ...
	  <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box box-primary">
            ...
            <ul class="nav nav-tabs">
              <li class="active"><a href="#ambil" data-toggle="tab">Perkembangan Kognisi</a></li> 
              <li><a href="#ds" data-toggle="tab">Perkembangan DS</a></li>
        ...
        <tr data-id="DataPsikomotor" >
                  ...
                  <input type="text" id="waktu" class="dataKognisi" data-idGejala="<?php echo "$a[idData]"?>" value="<?php foreach ($nilai_kognisi as $b) {if ($b['idData']==$a['idData']){print_r($b['waktu']); }};?> " name="waktu" placeholder="Masukkan waktu" data-namaKolom="waktu" data-tahap="2">
                </div></td>
                  <td><div class="form-group">
                  <input type="text" id="waktu" class="dataKognisi" data-idGejala="<?php echo "$a[idData]"?>" value="<?php foreach ($nilai_kognisi as $b) {if ($b['idData']==$a['idData']){print_r($b['keterangan']); }};?> " name="waktu" placeholder="Masukkan waktu" data-namaKolom="keterangan" data-tahap="2">
                </div></td>
        ...
        ?>
                <tr>
                  <td>Hasil</td>
                  <td></td>
                  <td><?php foreach ($nilai_kognisi as $b) {if ($b['idData']==$a['idData']){print_r($b['nilai']); }};?></td>
                </tr>
                </tbody>
        ...
        <tr data-id="DataDS" >
                  ...
                  <td><input type="radio" name="ds<?php echo "$a[idGejala]"?>" class="ds" data-idGejala="<?php echo "$a[idGejala]"?>" id="ds1" value="" <?php foreach ($nilai_ds as $b) {if ($b['idGejala']==$a['idGejala'] && $b['idPelanggan']==$idPelanggan){if ($b['nilai']==1){echo "checked";}}};?>></td>
                  <td><input type="radio" name="ds<?php echo "$a[idGejala]"?>" class="ds" data-idGejala="<?php echo "$a[idGejala]"?>" id="ds2" value="" <?php foreach ($nilai_ds as $b) {if ($b['idGejala']==$a['idGejala'] && $b['idPelanggan']==$idPelanggan){if ($b['nilai']==0){echo "checked";}}};?>></td>
  ...
  $(document).ready(function(){
        $(".kognisi").change(function() { 
          ...
          console.log(idKaryawan);
            if(psikomotor=="kognisi3") { 
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_kognisi2'); ?>",
                    data: { idPelanggan:idPelanggan, idKaryawan:idKaryawan, idData:idData, strState:30 }
                });
            } else if(psikomotor=="kognisi2") { 
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_kognisi2'); ?>",
  ...
  $(document).ready(function(){
        $(".dataKognisi").change(function() { 
          ...
          console.log(idData);
                ...
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_data_kognisi'); ?>",
  ...
  $(document).ready(function(){
        $(".ds").change(function() { 
          ...
          console.log(tunalaras);
            if(tunalaras=="ds1") { 
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_ds'); ?>",
