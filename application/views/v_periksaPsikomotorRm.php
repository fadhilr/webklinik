...
<div class="box box-primary">
            ...
            <ul class="nav nav-tabs">
              <li class="active"><a href="#ambil" data-toggle="tab">Perkembangan Psikomotor</a></li> 
              <li><a href="#rm" data-toggle="tab">Perkembangan RM</a></li>
        ...
        <tr data-id="DataPsikomotor" >
                  ...
                  <input type="text" id="nama" class="dataPsikomotor" data-idGejala="<?php echo "$a[idData]"?>" value="<?php foreach ($nilai_psikomotor as $b) {if ($b['idData']==$a['idData']){print_r($b['waktu']); }};?> " name="nama" placeholder="Masukkan waktu" data-namaKolom="waktu" data-tahap="2">
                </div></td>
                  <td><div class="form-group">
                  <input type="text" id="nama" class="dataPsikomotor" data-idGejala="<?php echo "$a[idData]"?>" value="<?php foreach ($nilai_psikomotor as $b) {if ($b['idData']==$a['idData']){print_r($b['keterangan']); }};?>" name="nama" placeholder="Masukkan keterangan" data-namaKolom="keterangan" data-tahap="2">
                </div></td>
        ...
        ?>
                <tr>
                  <td>Hasil</td>
                  <td></td>
                  <td><?php foreach ($nilai_psikomotor as $b) {if ($b['idData']==$a['idData']){print_r($b['nilai']); }};?></td>
                </tr>
                </tbody>
                <tfoot>
                <tr>
        ...
        <tr data-id="DataDS" >
                  ...
                  <td><input type="radio" name="rm<?php echo "$a[idGejala]"?>" class="rm" data-idGejala="<?php echo "$a[idGejala]"?>" id="rm1" value="" <?php foreach ($nilai_rm as $b) {if ($b['idGejala']==$a['idGejala'] && $b['idPelanggan']==$idPelanggan){if ($b['nilai']==1){echo "checked";}}};?>></td>
                  <td><input type="radio" name="rm<?php echo "$a[idGejala]"?>" class="rm" data-idGejala="<?php echo "$a[idGejala]"?>" id="rm2" value="" <?php foreach ($nilai_rm as $b) {if ($b['idGejala']==$a['idGejala'] && $b['idPelanggan']==$idPelanggan){if ($b['nilai']==0){echo "checked";}}};?>></td>
     ...
     $(document).ready(function(){
        $(".psikomotor").change(function() { 
          ...
          console.log(psikomotor);
            if(psikomotor=="psikomotor3") { 
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_psikomotor2'); ?>",
                    data: { idPelanggan:idPelanggan, idKaryawan:idKaryawan, idData:idData, strState:30 }
                });
            } else if(psikomotor=="psikomotor2") { 
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_psikomotor2'); ?>",
       ...
       $(document).ready(function(){
        $(".dataPsikomotor").change(function() { 
          var idPelanggan=<?php echo "$idPelanggan";?>;
          var idKaryawan=$('#idKaryawan').val();
          var idData= $(this).attr("data-idGejala");
          ...
          console.log(idData);
                ...
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_data_psikomotor'); ?>",
       ...
       $(document).ready(function(){
        $(".rm").change(function() { 
          ...
          console.log(tunalaras);
            if(tunalaras=="rm1") { 
                $.ajax({
                    url: "<?php echo base_url('c_karyawan/set_rm'); ?>",
